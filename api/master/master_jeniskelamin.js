...
router.post('/viewData', async (req, res, next) => {
  try {
    ...
    const totalItems = await master_jeniskelamin.countDocuments(filter);
...
router.post('/addData', async (req, res, next) => {
    ...
    const master_jeniskelamin = await getCollection('master_jeniskelamin');
    const result = await master_jeniskelamin.insertOne(data);
...
router.post('/editData', async (req, res, next) => {
    const data = req.body; 
    const master_jeniskelamin = await getCollection('master_jeniskelamin');
    const result = await master_jeniskelamin.updateOne({ id :data.id }, 
        { $set: { 
                    uraian         : data.uraian              
                }
        }) 
...
router.post('/removeData', async (req, res, next) => {
    const data = req.body;
    const master_jeniskelamin = await getCollection('master_jeniskelamin');
    const result = await master_jeniskelamin.deleteOne({ id: data.id });
