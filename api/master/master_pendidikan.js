...
router.post('/viewData', async (req, res, next) => {
  try {
    ...
    const totalItems = await master_pendidikan.countDocuments(filter);
...
router.post('/addData', async (req, res, next) => {
    ...
    const master_pendidikan = await getCollection('master_pendidikan');
    const result = await master_pendidikan.insertOne(data);
...
router.post('/editData', async (req, res, next) => {
    const data = req.body; 
    const master_pendidikan = await getCollection('master_pendidikan');
    const result = await master_pendidikan.updateOne({ id :data.id }, 
        { $set: { 
                    uraian         : data.uraian              
                }
        }) 
...
router.post('/removeData', async (req, res, next) => {
    const data = req.body;
    const master_pendidikan = await getCollection('master_pendidikan');
    const result = await master_pendidikan.deleteOne({ id: data.id });
