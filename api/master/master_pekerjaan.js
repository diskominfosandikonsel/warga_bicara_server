...
router.post('/viewData', async (req, res, next) => {
  try {
    ...
    const totalItems = await master_pekerjaan.countDocuments(filter);
...
router.post('/addData', async (req, res, next) => {
    ...
    const master_pekerjaan = await getCollection('master_pekerjaan');
    const result = await master_pekerjaan.insertOne(data);
...
router.post('/editData', async (req, res, next) => {
    const data = req.body; 
    const master_pekerjaan = await getCollection('master_pekerjaan');
    const result = await master_pekerjaan.updateOne({ id :data.id }, 
        { $set: { 
                    uraian         : data.uraian              
                }
        }) 
...
router.post('/removeData', async (req, res, next) => {
    const data = req.body;
    const master_pekerjaan = await getCollection('master_pekerjaan');
    const result = await master_pekerjaan.deleteOne({ id: data.id });
