...
router.post('/autocomplete', async (req, res, next) => {
  try {
    const master_kategori_laporan = await getCollection('master_kategori_laporan');
    ...
    const result = await master_kategori_laporan
      .find(filter)
      .sort({ urutan: 1 })
      .limit(10)
      .toArray();
...
router.post('/viewData', async (req, res, next) => {
  try {
    ...
    const totalItems = await master_kategori_laporan.countDocuments(filter);
...
router.post('/addData', async (req, res, next) => {
    ...
    const master_kategori_laporan = await getCollection('master_kategori_laporan');
    const result = await master_kategori_laporan.insertOne(data);
...
router.post('/editData', async (req, res, next) => {
    const data = req.body; 
    const master_kategori_laporan = await getCollection('master_kategori_laporan');
    const result = await master_kategori_laporan.updateOne({ id :data.id }, 
        { $set: { 
                    uraian         : data.uraian              
                }
        }) 
...
router.post('/removeData', async (req, res, next) => {
    const data = req.body;
    const master_kategori_laporan = await getCollection('master_kategori_laporan');
    const result = await master_kategori_laporan.deleteOne({ id: data.id });
